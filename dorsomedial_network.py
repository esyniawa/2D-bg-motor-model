...
dPFC = ann.Population(name="dPFC", geometry=model_params['nGoals'], neuron=BaselineNeuron)

StrD1_caud = ann.Population(name="StrD1", geometry=model_params['dim_Str'], neuron=LinearNeuron_trace)
StrD1_caud.noise = 0.01
StrD1_caud.lesion = 1.0

StrD2_caud = ann.Population(name="StrD2_caud", geometry=model_params['dim_Str'], neuron=LinearNeuron_trace)
StrD2_caud.noise = 0.01
StrD2_caud.lesion = 1.0

STN_caud = ann.Population(name="STN_caud", geometry=model_params['nBodySchema'], neuron=BaselineNeuron)
STN_caud.noise = 0.01

GPi = ann.Population(name="GPi", geometry=model_params['dim_BG'], neuron=LinearNeuron)
GPi.noise = 0.05
GPi.baseline = 1.9

GPe = ann.Population(name="GPe", geometry=model_params['dim_BG'], neuron=LinearNeuron)
GPe.noise = 0.05
GPe.baseline = 1.0

VA = ann.Population(name="Thal_caud", geometry=model_params['dim_BG'], neuron=LinearNeuron)
VA.noise = 0.025
VA.baseline = 0.7

StrThal_caud = ann.Population(name="StrThal_caud", geometry=model_params['dim_BG'], neuron=LinearNeuron)
StrThal_caud.noise = 0.01
StrThal_caud.baseline = 0.4

SNc_caud = ann.Population(name='SNc_caud', geometry=model_params['nGoals'], neuron=DopamineNeuron)
SNc_caud.exc_threshold = 0.2
SNc_caud.baseline = 0.1
SNc_caud.factor_inh = 1.0

# Projections
PFCdStrD1 = ann.Projection(pre=dPFC, post=StrD1_caud, target='exc', synapse=DAPostCovarianceNoThreshold_trace)
...
PFCdStrD2_caud.connect_all_to_all(weights = Normal(0.12,0.03)) #0.005
PFCdStrD2_caud.tau = 2000.0
PFCdStrD2_caud.regularization_threshold = 1.5
PFCdStrD2_caud.tau_alpha = 15.0
PFCdStrD2_caud.baseline_dopa = 8*baseline_dopa_caud
...
StrD2GPe = ann.Projection(pre=StrD2_caud, post=GPe, target='inh', synapse=DAPreCovariance_inhibitory)
StrD2GPe.connect_all_to_all(weights=(0.01)) # was 0.01
StrD2GPe.tau = 2500
StrD2GPe.regularization_threshold = 1.5
StrD2GPe.tau_alpha = 20.0
StrD2GPe.baseline_dopa = 0.5
StrD2GPe.K_dip = 0.1
